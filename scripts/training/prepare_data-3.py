...
def read_altos_from_metadata(df_metadata: pd.DataFrame, config: dict, logger: logging.Logger) -> dict:
    """
    ...
    """
    altos_paths, altos_ids, filenames, labels, sets, altos_ocr_flags, file_hashes = [], [], [], [], [], [], []
    n_missing = 0
    ...
    for label, df_label in df_metadata.groupby("label"):
        ...
        for row in df_label.itertuples():
            ...
            labels.append(label)
            sets.append(row.set)
            altos_ocr_flags.append(loaded_alto["OCRized"])
    ...
    return {
        ...
        "labels": labels,
        "sets": sets,
        "altos_ocr_flags": altos_ocr_flags,
        "file_hashes": file_hashes,
    }


def find_duplicate_groups(file_hashes: list, sets: list) -> list:
    """
    ...
    for i, hash in enumerate(file_hashes):
        idxs_by_hash.setdefault(hash, []).append(i)

    groups = []
    for idxs in idxs_by_hash.values():
        if len(idxs) < 2:
            continue
...
def log_duplicate_groups(groups: list, filenames: list, altos_ids: list, labels: list, sets: list, logger) -> None:
    """
    ...
    Args:
        groups (list): Output of find_duplicate_groups.
...
def build_stratify(labels: list, ocr_flags: list, logger: logging.Logger, context: str = ""):
    """
    ...
    undersized = {group: n for group, n in counts.items() if n < MIN_GROUP_SIZE}

    if not undersized:
        return joint

    logger.warning(
        f"{context}: cannot stratify on (label, ocr), {len(undersized)} group(s) below "
        f"{MIN_GROUP_SIZE} members ({undersized}). Falling back to label only."
    )

    label_counts = Counter(labels)
    undersized_labels = {label: n for label, n in label_counts.items() if n < MIN_GROUP_SIZE}
    if undersized_labels:
        logger.warning(
            f"{context}: cannot stratify on label either ({undersized_labels}). "
...
def prepare_data(config, logger):
    ...
    try:
        ...
        collected = read_altos_from_metadata(df_metadata, config, logger)
        altos_paths = collected["altos_paths"]
        altos_ids = collected["altos_ids"]
        filenames = collected["filenames"]
        labels = collected["labels"]
        sets = collected["sets"]
        altos_ocr_flags = collected["altos_ocr_flags"]
        ...
        duplicate_groups = find_duplicate_groups(file_hashes, sets)
        log_duplicate_groups(duplicate_groups, filenames, altos_ids, labels, sets, logger)

        duplicate_idxs = sorted(i for _, removed in duplicate_groups for i in removed)
        duplicate_labels = [labels[i] for i in duplicate_idxs]

        altos, altos_paths, altos_ids, filenames, labels, sets, altos_ocr_flags = remove_idxs(
            duplicate_idxs, altos, altos_paths, altos_ids, filenames, labels, sets, altos_ocr_flags
        )
        logger.info(f"Identified {len(duplicate_idxs)} duplicates and removed : {Counter(duplicate_labels)}")

        # Count and display OCR and non OCR per label for sanity check
        ocr_counter = Counter(zip(labels, altos_ocr_flags))
        ...
        altos_ocr_flags_test = [altos_ocr_flags[i] for i in test_idxs]

        altos_paths_pool = [altos_paths[i] for i in train_idxs]
        y_pool = [labels[i] for i in train_idxs]
        altos_ocr_flags_pool = [altos_ocr_flags[i] for i in train_idxs]
        ...
        stratify = build_stratify(y_pool, altos_ocr_flags_pool, logger, context="train/val split")
