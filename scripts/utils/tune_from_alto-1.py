...
def main():
    ...
    rows = []
    best_so_far = 0.0
    combo_iter = tqdm(all_combos, desc="Test des combinaisons")
    for preproc_params, engine_params in combo_iter:
        ...
        rows.append(row)
        best_so_far = max(best_so_far, row["success_rate"])
        combo_iter.set_postfix(best=f"{best_so_far*100:.1f}%")
