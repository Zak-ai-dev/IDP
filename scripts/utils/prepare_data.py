...
def split_test_dataset(df_docs: pd.DataFrame, config: dict, logger) -> pd.DataFrame:
    ...
    Optionally filtered on config['parcours_test'] (list of parcours, or "all" for no filter).
...
def split_validation_dataset(df_docs: pd.DataFrame, config: dict, logger) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    Only rows whose 'parcours' is in config['parcours_train'] are kept for train/validation
    (test dataset keeps its own filter via config['parcours_test'], unaffected by this one).
    ...
    if "train" in df_docs['set'].values:
        ...
        if parcours_train and parcours_train != 'all':
