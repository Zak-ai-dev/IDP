...
# Gère : cni_new_0157.pdf, cni_new_157.pdf, test_cni_new_0157.pdf, TRI_CNI_ETR_0297.pdf
...
# =============================================================================
# Filtrage par typologie (workspace multi-labels)
# =============================================================================
def filter_by_filename_regex(df: pd.DataFrame, label_cfg: dict, logger: logging.Logger) -> pd.DataFrame:
    """
    Ne conserve que les documents dont le filename correspond à la regex du label.
    Indispensable sur un workspace multi-labels (workspace explo 77) où plusieurs
    typologies cohabitent sous la nomenclature TRI_<TYPE>_XXXX.pdf.

    No-op si le label n'a pas de clé "filename_regex".

    Args:
        df (pd.DataFrame): Documents listés pour le workspace.
        label_cfg (dict): Entrée de config du label.
        logger (logging.Logger): Logger.

    Returns:
        pd.DataFrame: DataFrame restreint à la typologie du label.
    """
    ...
    logger.info(f"--- [{label}] filtre nomenclature '{pattern}' : {before} -> {len(df)} docs")
    return df
...
def assign_set(filename: str, split_mode: str = "strict"):
    ...
    Note:
        Le mode "random" n'est pas traité ici : il ne dépend pas du filename et est
        géré directement dans sample_train_test.
    """
...
def sample_random_split(
    ...
    """
    Échantillonnage pour les labels dont le filename ne porte aucune indication
    train/test (workspace explo 77, nomenclature TRI_<TYPE>_XXXX.pdf).

    Le test est tiré en premier, puis le train est tiré dans le reliquat : les deux
    lots sont donc disjoints par construction, sans risque de fuite entre les splits.

    Args:
        df (pd.DataFrame): Documents de la typologie.
        label (str): Label traité (log).
        n_train (int): Nombre de documents train souhaités.
        n_test (int): Nombre de documents test souhaités.
        seed (int): Graine aléatoire.
        logger (logging.Logger): Logger.

    Returns:
        pd.DataFrame: Échantillon avec colonne "set".
    """
    df = df.copy()

    n_test_eff = min(n_test, len(df))
    if n_test_eff < n_test:
        logger.warning(f"--- [{label}] seulement {len(df)} docs disponibles (test demandé : {n_test})")
    ...
    if n_train_eff < n_train:
        logger.warning(f"--- [{label}] seulement {len(df_rest)} docs restants (train demandé : {n_train})")
...
def sample_train_test(
    ...
    Args:
        df (pd.DataFrame): Documents listés pour le label (déjà filtrés par typologie).
        label_cfg (dict): Entrée de config du label.
    ...
    # Cas sans indication train/test dans le filename : split tiré au sort.
...
def download_dataset(
    ...
    export_path est dédié à un label, donc un dossier par typologie de document.
...
def load_data_from_wks(
    ...
    """
    Charge les données d'un workspace pour un ou plusieurs labels.

    Le listing MinIO n'est effectué qu'une seule fois, même si plusieurs labels
    partagent le workspace (cas du workspace explo 77) : chaque label isole ensuite
    sa typologie via sa "filename_regex", puis échantillonne et télécharge dans son
    propre dossier.
    ...
    Args:
        ...
        label_cfgs (list[dict]): Entrées de config des labels rattachés à ce workspace.
        environment (str): "PROD" ou "QUAL".

    Returns:
        list[pd.DataFrame]: Un DataFrame de métadonnées par label chargé.
    """
    labels = [cfg["label"] for cfg in label_cfgs]
    logger.info(f"***** Début du chargement - workspace {workspace_id} - labels {labels} ...")
    ...
    try:
        ...
        # Le statut est porté par le workspace : on prend celui du premier label
        # et on alerte si les labels d'un même workspace divergent.
        ...
        if any(cfg.get("target_status", default_status) != target_status for cfg in label_cfgs):
            logger.warning(
                f"--- Workspace {workspace_id} : statuts divergents entre labels, "
                f"utilisation de {target_status}"
            )
        ...
        for label_cfg in label_cfgs:
            ...
            if df_label.empty:
                logger.warning(f"--- [{label}] aucun document après filtre nomenclature.")
                continue
        ...
        logger.info(f"***** Fin du chargement - workspace {workspace_id} ...")
        return dfs

    except Exception as e:
        logger.error(f"Erreur sur le workspace {workspace_id} (labels {labels}) : {e}")
...
def load_data(config: dict, logger: logging.Logger, environment: str = "PROD") -> pd.DataFrame:
    ...
    - Un seul listing MinIO par workspace, même lorsque plusieurs labels le partagent.
